...
async function fetchTransactions(): Promise<Transaction[]> {
  try {
    ...
    const typedPurchases: Transaction[] = purchases.map((p: any) => ({
      ...p,
      type: "purchase",
    }));
    const typedDeposits: Transaction[] = deposits.map((d: any) => ({
      ...d,
      type: "deposit",
    }));
...
async function updateTransaction(transactionData: Partial<Transaction>) {
  ...
  const endpoint =
    type === "purchase" ? "/api/user/edit/purchase" : "/api/user/edit/deposit";
...
async function deleteTransaction(transaction: Transaction) {
  ...
  const endpoint =
    type === "purchase"
      ? "/api/user/delete/purchase"
      : "/api/user/delete/deposit";
  ...
  const filteredTransactions = React.useMemo(() => {
    let transactions = allTransactions.filter((t) => {
      const matchAccount =
        ...
        String(t.bankAccountId) === selectedAccountId;
      const matchName =
        !filters.name ||
        t.title.toLowerCase().includes(filters.name.toLowerCase());
  ...
  const handleSave = async (updatedData: Partial<Transaction>) => {
    try {
      ...
      setAllTransactions((prev) =>
        prev.map((t) =>
          t.id === updatedData.id && t.type === updatedData.type
            ? { ...t, ...updatedData }
            : t
        )
      );
        ...
        {isMobile ? (
          <Sheet open={isFilterSheetOpen} onOpenChange={setFilterSheetOpen}>
            <SheetTrigger asChild>
              <Button
                variant="outline"
                className="w-full md:w-auto bg-white/80"
              >
                ...
                {paginatedTransactions.length > 0 ? (
                  paginatedTransactions.map((t) => (
                    <TableRow key={`${t.type}-${t.id}`}>
                      ...
                      <TableCell>
                        <Badge
                          variant={t.type === "deposit" ? "success" : "outline"}
                        >
                          {t.type}
                        </Badge>
                      </TableCell>
